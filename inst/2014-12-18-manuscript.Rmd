...
abstract: |
  ...
  when diseases are load-dependent may underestimate long-term effects. **effects of what type**
---
...
**This paragraph seems thin, maybe add some theoretical component or join with next p'graph**
...
disease as a binary state of the host [@Kermack1927]. **Need some citations to back this up, examples of these "most attempts"**
Extensions such as
susceptible-exposed-infected-removed (*SEIR*) models represent disease
progression within hosts, but do not capture the accumulation of new **avoid italics for emphasis**
...
smaller literature on age structure in multi-infectoin models. **run spellcheck** In
...
**While techinically correct, this isn't a typical use of the term null model, at least in ecology. I would say it like:**
...
$i$. Trees **trees?** advance to the next disease class at rate $\Lambda$, the overall
...
$\text{param}_{SI}$, $\text{param}_{SIV}$, or $\text{param}_\text{multi}$ **seems like a non-standard way-- just say subscripted with $SI$ etc** when
...
assumptions of the reduced model break down in the presence of age structure. **Which key assumptions? Concisely state here (if not obvious can cite your unpublished calculations that lead to this?)**
...
**Unclear if there is 1, 2 or 4 constants. Below it seems there is one constant per model, so $c_SIV$ and $c_multi$**
...
2.  **Initial growth and acceleration rates of infected individuals.** Next, $c$
    ...
    individuals, is identical at all cases under these initial conditions. **Omit the first condition or state is in text rather than display equation (since it's trivial)** Thus,
...
**Refocus: Pattern of mortality rate over time in epidemic**

**Models with similar equilibrium mortality rate differ in initial transient
mortality rate**. Figure 1 shows the dynamic behavior of the $SI$, $SIV$, and
...
**Need descriptive figure captions -- include meanings of lines, axes etc.
Also  differences between SI, SIV, multi this would be clearer if linetype was keyed to model**
...
**Fig 2 shows multi and Fig 3 shows SIV in the pdf. The point is to show the number of individuals in the more-infected classes increases over time in both models. This should probably be one figure  **
...
time until 10% of the total population was infected. As with the last
...
**As stated elsewhere, I think this effect of age-dependent infection in influcing mortality rates is the most generally interesting aspect of this work. However, fig 2 and 3 aren't very good graphics to illustrate this point. Rather, they most clearly show that the there are greater proportions of more-infected individuals (in all age classes) as the disease establishes. Secondarily, one can notice that the proportion is higher in adults. It might be better to show this latter point more clearly, eg by comparing two time points with stacked bar chart. Also, a figure that shows these mortality effects is needed (and should be a simple transformation of the data underlying these figures, given parameters d and alpha)**
